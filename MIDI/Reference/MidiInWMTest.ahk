...
midiInHandler(hInput, midiMsg, wMsg) 
{
	...
	#InputLevel 1
	;MsgBox, yeet
	FileDelete, %A_WorkingDir%\midibyte.txt
	FileAppend, %byte1%, %A_WorkingDir%\midibyte.txt
	Send {VK0x87}
	if (byte1 = 104)
	{
		;MsgBox, yeet
		...
		SetTitleMatchMode, 2
		path = C:\Users\humerous\Desktop\AutoHotKey\MIDITEST.ahk
		WinClose, %path%
		;MsgBox, yeet
		return
	}
...
;if (%byte1% = 0)
	;{
		;ControlSend, ahk_parent, 1x1, MIDITranslator.exe
		;ControlSendRaw, Edit1, 1x1, notepad.exe, Untitled - Notepad
		;MsgBox, Sent
	;}
